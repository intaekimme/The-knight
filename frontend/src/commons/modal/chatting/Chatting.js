...
import {
  ...
  chattingMine,
  chattingOthers,
} from "../../../_css/ChattingCSSProperties";
...
export default function Chatting(props) {
  ...
  // 채팅 색깔
  const [chattingColor, setChattingColor] = React.useState("#4d4d4d");
  // 입력 메세지
  ...
  React.useEffect(() => {
    for (let i = 0; i < members.length; i++) {
      if (
        members[i].id.toString() === window.localStorage.getItem("memberId")
      ) {
  ...
  React.useEffect(() => {
    ...
    if (
      chattingBodyElement &&
      chatScrollHeight - chatScrollTop < chattingModalHeight + 10
    ) {
    ...
    <div sx={{ position: "fixed", bottom: 0, left: 0 }}>
      {open ? (
        ...
        <Button
          ...
          sx={{ ...chattingPosition, color: black, width: 120 }}
        >
          <QuestionAnswerOutlinedIcon sx={{ fontSize: 70, color: white }} />
        </Button>
      )}
      <Modal
        open={open}
        ...
        <Box sx={{ ...chattingPosition }}>
          ...
          >
            ...
            >
              ...
              >
                <Button
                  onClick={onChattingClose}
                  sx={{ color: red }}
                >
                  <MinimizeIcon sx={{ fontSize: size / 2 }} />
                </Button>
              </Grid>
            </Grid>
            <Grid
            ...
            >
              ...
              {chattingLog.map((chat, index) => {
                  ...
                  const innerHTML = `<div>${chat.content
                    .toString()
                    .replaceAll("\n", "</div><div>")}</div>`;
            ...
            >
              <Grid
                item
                xs={12}
                sx={{ textAlign: "left", background: "#ADADAD" }}
              >
                <Box
                  sx={{
                    fontSize: size / 1.7,
              ...
              <Grid
                ...
                item
                xs={12}
                sx={{ ...chattingInput, textAlign: "left", paddingTop: 1 }}
              >
                <Grid
                  item
                  xs={9}
                ...
                <Grid
                  item
                  xs={3}
                  sx={{ pb: 1, pr: 1, textAlign: "right" }}
                >
                  <Button
                    onClick={onMessageSend}
                    sx={{ ...chattingSendButton, fontSize: size / 3 }}
                  >
                    보내기
                  </Button>
                </Grid>
