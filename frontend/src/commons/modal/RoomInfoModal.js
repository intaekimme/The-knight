		...
		>
			<Box sx={{...modalStyle, backgroundImage:`url(${modalBackground})`, backgroundSize: 'cover'}}>
				<Box id="modal-modal-title">
					<Grid container alignItems={'center'} sx={{p:4, pt:0,}}>
							...
							<Grid item xs={10} sx={{pl:4, pr:4}}>
								<Box sx={{ minWidth: 120 }}>
									<FormControl fullWidth>
										...
										>
											<MenuItem value={4} sx={{fontSize: 25}}>2 vs 2</MenuItem>
											<MenuItem value={6} sx={{fontSize: 25}}>3 vs 3</MenuItem>
											<MenuItem value={8} sx={{fontSize: 25}}>4 vs 4</MenuItem>
											<MenuItem value={10} sx={{fontSize: 25}}>5 vs 5</MenuItem>
										</Select>
									</FormControl>
								</Box>
							</Grid>
						...
						<Grid container columns={24} item xs={24} sx={{mt:1, mb:1}}>
							...
							{items.map((item, index)=>(
								<Grid container columns={24} item xs={5} key={`item${item}`}>
									<Grid columns={12} item xs={12}><ItemBox size={190} textColor="#DCD7C9" buttonDisabled={true}/></Grid>
				...
				:
				<Grid item xs={12} alignItems="center" sx={{mt:5, textAlign:"center"}}><Button variant="outlined" sx={buttonStyle} onClick={onConfirm}>입장</Button></Grid>
